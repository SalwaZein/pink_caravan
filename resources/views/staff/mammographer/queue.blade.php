...
@php
    // Two lists, same row shape: cases routed here now, and cases still awaiting
    // their mammography report (which may only arrive days after the visit).
    $cols = '150px 1.4fr 110px 130px 110px';
@endphp
...
<x-staff-shell :role="$sidebarRole" :route="$route">
    ...
    @foreach ([
        ['title' => __('pc.mammo_assigned_cases'), 'hint' => null, 'rows' => $rows, 'empty' => __('pc.no_completed_records')],
        ['title' => __('pc.mammo_awaiting_report'), 'hint' => __('pc.mammo_awaiting_hint'), 'rows' => $pendingRows, 'empty' => __('pc.mammo_awaiting_empty')],
    ] as $section)
