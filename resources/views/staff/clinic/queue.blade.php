...
<x-staff-shell :role="$sidebarRole" :route="$route">
    <div class="pc-anim">
        ...
        <div style="background:#fff;border:1px solid #EFE2EA;border-radius:16px;overflow:hidden;box-shadow:0 3px 14px rgba(120,60,90,.05);">
            ...
            @foreach ($queue as $p)
                <div style="display:grid;grid-template-columns:150px 1.4fr 60px 130px 130px 100px;gap:12px;align-items:center;padding:13px 20px;border-top:1px solid #F3E7EE;font-size:13.5px;">
                    ...
                    <div style="text-align:end;"><a href="{{ url('/clinic/assign') }}" role="button" style="cursor:pointer;font-size:12.5px;font-weight:700;color:#E6017E;padding:6px 12px;border:1px solid #F6BFD9;border-radius:8px;text-decoration:none;">{{ __('pc.assign') }}</a></div>
                </div>
            @endforeach
        </div>
    </div>
